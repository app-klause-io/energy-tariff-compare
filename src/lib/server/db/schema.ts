import { pgTable, uuid, text, timestamp, integer, boolean, numeric, date } from 'drizzle-orm/pg-core';
...
export const maintenanceLogRelations = relations(maintenanceLog, ({ one }) => ({
	...
	complianceItem: one(complianceItems, { fields: [maintenanceLog.complianceItemId], references: [complianceItems.id] })
}));
